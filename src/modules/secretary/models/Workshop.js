...
module.exports = () => {
  ...
  const find = async id => {
    ...
    `${TABLE_NAME}.nome`,
    `${TABLE_NAME}.idade_minima`,
    `${TABLE_NAME}.idade_maxima`,
    `${TABLE_NAME}.outras_restricoes`,
    `${TABLE_NAME}.atividade`
    ).from(TABLE_NAME).first().then(row => row)
    if (!result) { throw new Error('Workshop not found') }
